...
public partial class RoutePlannerPageModel : ObservableObject
{
    ...
    {
        // Ensure we use the property names (SelectedOrigin) generated by [ObservableProperty]
        ...
        try
        {
            // Call the method in your ApiService
            var route = await _api.GetShortestRouteAsync(SelectedOrigin.stop_id, SelectedDestination.stop_id);

            CalculatedRoute.Clear();
            foreach (var s in route) CalculatedRoute.Add(s);

            // Manual notification since CalculatedRoute is a collection, not a property
